...
describe('DomainClient', function() {
    ...
    describe('updateDKIMSelector', () => {
        it('updates the DKIM selector for a domains', () => {
            api.put('/v3/domains/test.example.com/dkim_selector?dkim_selector=dkim_selector_value')
               .reply(200, { message: 'Domain DKIM selector updated' });

            return client.updateDKIMSelector('test.example.com', { dkimSelector: 'dkim_selector_value' })
                         .then((response: UpdatedDKIMSelectorResponse) => {
                             response.should.eql(
                                 {
    ...
    describe('updateWebPrefix', () => {
        it('Update the CNAME used for tracking opens and clicks', () => {
            api.put('/v3/domains/test.example.com/web_prefix?web_prefix=webprefixvalue')
               .reply(200, { message: 'Domain web prefix updated' });

            return client.updateWebPrefix('test.example.com', { webPrefix: 'webprefixvalue' })
                         .then((response: UpdatedWebPrefixResponse) => {
                             response.should.eql(
                                 {
